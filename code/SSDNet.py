    ...
    with tf.variable_scope('ObjectDetector'):
        with tf.variable_scope('Detector1'):
            with tf.variable_scope('Location'):
                ...
                loc1 = tf.reshape(loc1, shape=[-1, para.FEATURE_MAPS[0][0] * para.FEATURE_MAPS[0][1] * 12, 8])
            with tf.variable_scope('Classification'):
                filters = (para.NUM_CLASSESS) * 12
                cls1 = tf.layers.conv2d(net1, filters, kernel_size=(1, 1), strides=(1, 1), padding='same', activation=None, use_bias=True)
                cls1 = tf.reshape(cls1, shape=[-1, para.FEATURE_MAPS[0][0] * para.FEATURE_MAPS[0][1] * 12, (para.NUM_CLASSESS)])
        with tf.variable_scope('Detector2'):
            with tf.variable_scope('Location'):
                ...
                loc2 = tf.reshape(loc2, shape=[-1, para.FEATURE_MAPS[1][0] * para.FEATURE_MAPS[1][1] * 12, 8])
            with tf.variable_scope('Classification'):
                filters = (para.NUM_CLASSESS) * 12
                cls2 = tf.layers.conv2d(net2, filters, kernel_size=(1, 1), strides=(1, 1), padding='same', activation=None, use_bias=True)
                cls2 = tf.reshape(cls2, shape=[-1, para.FEATURE_MAPS[1][0] * para.FEATURE_MAPS[1][1] * 12, (para.NUM_CLASSESS)])
        with tf.variable_scope('Detector3'):
            with tf.variable_scope('Location'):
                ...
                loc3 = tf.reshape(loc3, shape=[-1, para.FEATURE_MAPS[2][0] * para.FEATURE_MAPS[2][1] * 12, 8])
            with tf.variable_scope('Classification'):
                filters = (para.NUM_CLASSESS) * 12
                cls3 = tf.layers.conv2d(net3, filters, kernel_size=(1, 1), strides=(1, 1), padding='same', activation=None, use_bias=True)
                cls3 = tf.reshape(cls3, shape=[-1, para.FEATURE_MAPS[2][0] * para.FEATURE_MAPS[2][1] * 12, (para.NUM_CLASSESS)])
        with tf.variable_scope('Detector4'):
            with tf.variable_scope('Location'):
                ...
                loc4 = tf.reshape(loc4, shape=[-1, para.FEATURE_MAPS[3][0] * para.FEATURE_MAPS[3][1] * 12, 8])
            with tf.variable_scope('Classification'):
                filters = (para.NUM_CLASSESS) * 12
                cls4 = tf.layers.conv2d(net4, filters, kernel_size=(1, 1), strides=(1, 1), padding='same', activation=None, use_bias=True)
                cls4 = tf.reshape(cls4, shape=[-1, para.FEATURE_MAPS[3][0] * para.FEATURE_MAPS[3][1] * 12, (para.NUM_CLASSESS)])
        with tf.variable_scope('Detector5'):
            with tf.variable_scope('Location'):
                ...
                loc5 = tf.reshape(loc5, shape=[-1, para.FEATURE_MAPS[4][0] * para.FEATURE_MAPS[4][1] * 12, 8])
            with tf.variable_scope('Classification'):
                filters = (para.NUM_CLASSESS) * 12
                cls5 = tf.layers.conv2d(net5, filters, kernel_size=(1, 1), strides=(1, 1), padding='same', activation=None, use_bias=True)
                cls5 = tf.reshape(cls5, shape=[-1, para.FEATURE_MAPS[4][0] * para.FEATURE_MAPS[4][1] * 12, (para.NUM_CLASSESS)])
        with tf.variable_scope('Detector6'):
            with tf.variable_scope('Location'):
                ...
                loc6 = tf.reshape(loc6, shape=[-1, para.FEATURE_MAPS[5][0] * para.FEATURE_MAPS[5][1] * 11, 8])
            with tf.variable_scope('Classification'):
                filters = (para.NUM_CLASSESS) * 11
                cls6 = tf.layers.conv2d(net6, filters, kernel_size=(1, 1), strides=(1, 1), padding='same', activation=None, use_bias=True)
                cls6 = tf.reshape(cls6, shape=[-1, para.FEATURE_MAPS[5][0] * para.FEATURE_MAPS[5][1] * 11, (para.NUM_CLASSESS)])
